...
# Load patterns
with open("/home/elk/PowerShellExaminer/powershell_code_patterns.json", "r") as f:
...
class PowerShellExaminerAnalyzer(Analyzer):
    ...
    def scan_code_patterns(self, target_file):
        ...
        self.scan_patterns()
        self.extract_urls()
        self.extract_domains()  # Use tldextract for domains
    ...
    def extract_domains(self):
        domains = set()
        for line in self.all_strings:
            extracted_domain = tldextract.extract(line)
            # Only add if a valid domain is found
            if extracted_domain.domain and extracted_domain.suffix:
                full_domain = f"{extracted_domain.domain}.{extracted_domain.suffix}"
                domains.add(full_domain)
        self.results['domains'] = list(domains)
